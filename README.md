...
- **真模型回归覆盖极薄**：集成测试用本地假 OpenAI 兼容服务（确定性、零成本），真模型只有 `tests/smoke/` 三条主链路，且需显式开启才外呼（见下），不进主 CI
...
**真模型冒烟**（三条主链路：纯问答 / 工具调用 / 检索）可以两种方式触发：

```bash
# 本地：需要显式开关——它会计费，不应混进"跑一遍全量测试"这种日常动作
PRAY_SMOKE=1 DEEPSEEK_API_KEY=sk-xxx python -X utf8 -m pytest tests/smoke -m smoke -q -s

# CI：.github/workflows/smoke.yml（每日定时 + 手动触发；未配置 secret 则跳过）
```

默认 `addopts` 含 `-m "not smoke"`，且用例内部还有 `PRAY_SMOKE=1` 这道闸——两道都是为了同一个目的：真实外呼必须是被显式要求的行为（`config.py` 在导入期 `load_dotenv()`，本机 `.env` 的真实 Key 会进入环境变量，只靠"检测到 Key 就跑"会让人在本地随手付费）。
...
| 文档 | 内容 |
|---|---|
| [docs/安全模型.md](docs/安全模型.md) | 命令分级三层判定、默认拒绝的理由、nonce 两阶段授权、来源校验、已知残留面与部署建议 |
| [docs/工程质量.md](docs/工程质量.md) | CI 首跑暴露的问题、测试分层与密封机制、覆盖率缺口明细、门禁取值理由 |
| [docs/改动与验证指南.md](docs/改动与验证指南.md) | 各项改动"在哪里能体现"：可验证的命令、实测输出、尚缺前端入口的功能清单 |
| [docs/本机使用指南.md](docs/本机使用指南.md) | MCP 接入 Claude Desktop、桌面版运行与打包的逐步操作 |
...
- **真模型覆盖仍很薄**：`tests/smoke/` 只有三条主链路（纯问答 / 工具调用 / 检索），与假服务下的 301 条用例相比可以忽略；prompt 与真实模型行为的偏差只能靠人工观察
