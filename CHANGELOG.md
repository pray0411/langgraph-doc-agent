...
### Fixed（第四轮外部评审 · 结果对齐与指标可信度）

评审的核心指控是"**证据与宣称不匹配**"：安全与测试基建是真功夫，但有两处
代码把启发式当成了指标在卖。逐条核实后修复（其中"依赖装不上"一条已在此前
修好，本轮补上防回归的 CI 门禁）：

- **【P1】工具结果与调用改为按 `tool_call_id` 对齐**：原实现是
  `zip(calls, results, strict=False)` 的**位置对齐**——一次调用若没有产出
  ToolMessage（异常中断、流式截断、并发工具节点乱序），**后续所有结果整体错位**，
  表现为来源卡片张冠李戴、grounded 取到别的工具的输出。`strict=False` 只保证
  不抛异常，不保证对齐正确。现按 id 配对，缺失的那条留空；旧数据无 id 时降级为
  位置配对并在返回项上标记 `matched_by="position"`（降级必须可见）。
  reflection 新增"工具结果对齐方式"字段。
- **【P1】`_grounded` 从"近乎恒真"改为可核验的量化判据**：原判据是"工具结果里
  任意一段 10 字符片段出现在回答中"——一段 500 字的结果能滑出约 490 个候选，
  命中一句通用短语即判真，实际接近恒为真。现改为：**剥离工具模板文本**（来源
  标注行、执行状态行等）后，取净内容与回答的**最长公共子串**，阈值 16 字符；
  `grounded_detail()` 同时返回 overlap 长度、命中片段、阈值与方法名，并在
  reflection 的说明里明确标注这是**启发式**（衡量文本复用程度，不代表事实正确性）。
- **【P1】检索阈值由"等于没有过滤"改为有意义的默认值**：`MIN_SCORE` 默认
  0.0 → 0.01（RRF 分恒为正，`score > 0.0` 不过滤任何结果，弱命中被静默塞给
  模型）；新增 `RETRIEVAL_LOW_CONFIDENCE`（默认 0.02），最高分低于它时
  `search_documents` 会在结果前显式提示"相关度较低，若不足以回答请如实说明"。
  返回项同时标注片段来自**知识库**还是**上传文档**。
- **【P1】上传通道与全局通道合并改为通道内归一化**：全局分是 BM25 与语义两个
  子通道之和（最高 ≈0.033），上传分只有单通道（最高 ≈0.017），直接比较会让
  上传文档被系统性压制——此前 README 写的"同量纲公平竞争"名不副实。现按通道内
  最高分归一后再排序，返回项带 `channel` 字段。
- **【P2】测试的环境耦合**：`test_chunk_overlap_invariant_is_enforced` 的子进程
  显式注入 `sys.path`，兼容安全路径模式（`python -P` / `PYTHONSAFEPATH=1`）——
  此前在这类环境下会 ImportError，失败信息却指向"配置不变式未生效"，把环境差异
  误报成产品缺陷；`_read_version` 的用例覆盖"解包目录优先、exe 同级目录兜底"；
  数值环境变量回退用例改为与"未设置时的默认值"比较，不再写死具体数字。

### Added（第四轮外部评审 · 补齐验证手段）

- **CI 新增 `deps-installable` job**：四个 requirements 文件都必须能被 pip 解析。
  这条来自一次真实事故——`requirements-mcp.txt` 曾因 `mcp==1.22.0` 与
  `fastmcp==4.0.3` 依赖链冲突**根本装不上**，而本机因为早已装好相关包毫无察觉。
  "照文档装依赖"必须是被门禁守着的不变量。
- **新增真模型冒烟**（`tests/smoke/` + `.github/workflows/smoke.yml`）：三条主链路
  （纯问答 / 工具调用 / 检索）跑真实模型，定时与手动触发，未配置密钥时整体跳过。
  此前全部集成测试都跑在本地假 OpenAI 服务上，证明不了"真实模型能按预期使用这些
  工具"。用例带显式开关 `PRAY_SMOKE=1`，且默认 `addopts` 含 `-m "not smoke"`——
  防止开发者本地随手跑全量测试时意外产生真实费用（`config.py` 在导入期
  `load_dotenv()`，本机 `.env` 的真实 Key 会进入 `os.environ`）。

### Changed（第四轮外部评审 · 定位与文档）

- README 重定位为"**一个本地 Agent 的工程实践：安全边界、测试基建与交付形态**"，
  并明确写出"Agent 内核没有技术增量"——避免按"Agent 创新"评估这个项目。
- 能力表改为"**能力 + 验证状态**"两列：每条能力标注它被验证到什么程度，
  没有验证手段的宣称不再写入。新增"**验证边界**"一节，主动披露：真模型回归
  极薄、评估集仅 8 篇手写语料、覆盖率数字带环境前提（未装 sentence-transformers）、
  无外部用户与贡献、前端单文件渲染器仅 E2E 金路径覆盖。
- 检索章节明确"混合"的依赖前提：**打包版 `Pray.spec` 默认排除
  sentence-transformers，即官方发布版检索为纯 BM25**；`retrieval_mode()` 是唯一
  可信的模式来源。
